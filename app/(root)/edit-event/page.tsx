...
import {
  Availability,
  AvailabilityTracker,
} from "@/components/layout/availabilityfield";
...
interface Event {
  ...
  latitude: number;
  longitude: number;
  preferredTraits: { name: string }[];
  preferredGoals: { name: string }[];
...
export default function EditEvent() {
  ...
  useEffect(() => {
    if (!session || status !== "authenticated" || !isOrganization) {
      ...
      setEventId(url.searchParams.get("eventID"));
      if (eventId) {
        console.log(`Event ID found: ${eventId}`); // Log the event ID
        setEventId(eventId); // Set the event ID in state
        readEvent(eventId); // Fetch event data using the event ID
      } else {
  ...
  const updateValues = (event: Event) => {
    if (load === 0) {
      ...
      const formattedDate =
        localDate.slice(11, 15) + "-" + month + "-" + localDate.slice(8, 10);
      ...
      const traits = event.preferredTraits ? event.preferredTraits.map((trait: { name: string }) => trait.name) : [];
      const goals = event.preferredGoals ? event.preferredGoals.map((goal: { name: string }) => goal.name) : [];

      setSelectedTraits(traits);
      setSelectedGoals(goals);

      // Directly use the provided preferredAvailability
      const morningAvailability: Availability = event.preferredAvailability.morningAvailability;
      const eveningAvailability: Availability = event.preferredAvailability.eveningAvailability;

      setMorningAvailability(morningAvailability);
      setEveningAvailability(eveningAvailability);
    }
  };
  ...
  const readEvent = async (eventID: string) => {
    ...
    if (result === null || result.organization_id !== organizationID) {
      ...
      updateValues(result); // Directly use the fetched event data
  ...
  const setAddress = (index: number) => {
    ...
    document
      .getElementById("dropdown-" + index)
      ?.classList.add("brightness-105");
    document
      .getElementById("dropdown-" + index)
      ?.classList.remove("hover:bg-gray-100");
    document
      .getElementById("dropdown-" + index)
      ?.classList.add("hover:bg-opacity-60");
  ...
  if (validateSubmit()) {
      ...
      console.log("Event Info: ", eventInfo); // Log event info
      ...
      if (!res.ok) {
        ...
        response.textContent = `Event update failed: ${errorMessage}. Please try again.`;
  ...
  return (
    ...
    >
      <div id="bodyContainer" className="flex w-10/12 flex-col">
        ...
        >
          ...
          >
            <div id="eventNameSubContainer" className="flex w-5/6 flex-col">
              ...
              <input
                ...
                value={valueName}
                className="text-ellipsis rounded-lg border border-2 border-[#EAEAEA] pl-6 text-gray-800 sm:text-lg "
          ...
          >
            <div
              ...
              className="flex w-1/3 min-w-36 flex-col pl-6 mb:ml-12 mb:pl-0 sm:min-w-52 md:ml-16 md:px-0 md:pr-8"
            >
              <label
              ...
              <input
                ...
                value={valueLocation}
                className="min-w-32 rounded-lg border border-2 border-[#EAEAEA] pl-3 text-sm text-gray-800 mb:ml-2 mb:pl-4 sm:text-lg"
          ...
          >
            <div className="mt-8 flex w-4/5 flex-col sm:w-1/3 sm:min-w-40">
              ...
              <input
                ...
                className="min-w-34 rounded-lg border border-2 border-[#EAEAEA] text-center text-sm font-semibold text-gray-800 sm:px-3 sm:text-base md:px-6"
                value={valueDate}
              ></input>
            </div>
            ...
            <div className="mt-8 flex w-4/5 flex-col sm:w-min lg:w-1/3">
              ...
              <div className="flex w-full flex-row rounded-lg border border-2 border-[#EAEAEA] bg-white p-0.5">
                <input
                  type="time"
                  id="startTime"
            ...
            >
              ...
              >
                ...
                <input
                  ...
                  className=" rounded-lg border border-2 border-[#EAEAEA] pl-6 text-center text-xl font-semibold text-gray-800 sm:min-w-40 sm:text-2xl"
          ...
          <div className="flex w-full justify-evenly pt-2">
            <div className="flex w-4/5 flex-col">
              ...
              <textarea
                ...
                className="max-h-44 min-h-36 rounded-lg border border-2 border-[#EAEAEA] pl-3 font-semibold text-gray-800"
              ></textarea>
            </div>
          </div>
          ...
          <div className="mt-8 flex w-full justify-evenly">
            <div className="flex w-4/5 flex-col">
              ...
              <input
                ...
                className="rounded-lg border border-2 border-[#EAEAEA] pl-3 font-semibold text-primary"
        ...
        <div
          id="closeEventBox"
          className=" absolute  mt-56 flex h-0 w-5/6 flex-col items-center overflow-hidden rounded-lg border-0 border-primary bg-gray-50 sm:mt-48 md:mt-64 lg:mt-52"
        >
          <div className="mt-8 h-fit w-5/6">
            <p className="text-center font-display text-4xl font-bold !text-secondary md:text-5xl xl:text-6xl">
              This event has ended.
            </p>
            <p className="mt-4 text-center font-display text-2xl font-bold !text-secondary md:text-3xl xl:text-4xl">
              Close the event to mark it as complete.
            </p>
            <h3 className="mt-8 text-center font-display text-lg italic !text-secondary opacity-60 xl:text-2xl">
              *This will allocate the token bounty{" "}
          ...
          <div className="mb-8 mt-12 flex w-full justify-evenly">
            <Link href="/my-events" className="w-1/3 md:w-1/5">
              <button className="text-md h-12 w-full rounded-md bg-secondary bg-opacity-60 font-semibold text-white transition-all duration-300 hover:opacity-80 focus:outline-none ">
                Cancel
              </button>
            </Link>
            <button
              ...
              className="text-md h-12 w-1/3 rounded-md bg-primary font-semibold text-white hover:opacity-80 focus:outline-none md:w-1/5"
            >
              Close Event
            </button>
          </div>
